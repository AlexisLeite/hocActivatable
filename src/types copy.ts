...
export type TApiaFormElementOption = {
  label: string;
  value: string;
};

export type TApiaFormElement = {
  onChange: string;
  mandatory: boolean;
  readonly: boolean;
  name: string;
...
export type TApiaFunction<FunctionType = TApiaTableFunction> = {
  function: {
    name: string;
...
export type TApiaLoad<T extends Record<string, unknown> = TApiaLoadForm> = {
  canClose: boolean;
  type: string;
} & T;
